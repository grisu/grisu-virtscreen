...
public class GoldJobNew {
	...
	static {
		...
		try {
			InputStream in = GoldJobNew.class.getResourceAsStream("/gold.sh");

			IOUtils.copy(in,
					new FileOutputStream(VIRTSCREEN_JOB_CONTROL_SCRIPT));

			in = GoldJobNew.class.getResourceAsStream("/gold.py");
	...
	private final GoldConfFileNew goldConfFile;
	...
	public GoldJobNew(ServiceInterface si, GoldConfFileNew confFileTemplate)
	...
	public void setCustomDockingAmount(int amount) {
		this.goldConfFile.getLigandFile().setLigandDockingAmount(amount);
	}

	public void setCustomLibraryFiles(String[] files) {
		this.goldConfFile.getLigandFile().setLigandFiles(files);
	}
